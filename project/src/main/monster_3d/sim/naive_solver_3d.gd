...
var monsters: Array[SimMonster3D] = []
var scanners_by_monster: Dictionary[SimMonster3D, Array] = {}
var boards_by_monster: Dictionary[SimMonster3D, ScannerBoard3D] = {}
...
func _process(_delta: float) -> void:
	...
	while not scanners_by_monster.is_empty():
		...
		var monster: SimMonster3D = monsters[monster_index]
		var scanners: Array[NaiveScanner3D] = scanners_by_monster[monster]
		var scanner: NaiveScanner3D = scanners.front()
		
		var board: ScannerBoard3D = boards_by_monster.get(monster)
		if board == null:
...
func create_board(monster: SimMonster3D) -> ScannerBoard3D:
	var board: ScannerBoard3D = ScannerBoard3D.new(monster)
	boards_by_monster[monster] = board
	for next_scanner: NaiveScanner3D in scanners_by_monster[monster]:
		next_scanner.board = board
	return board


func is_move_requested(monster: SimMonster3D) -> bool:
	return scanners_by_monster.has(monster)


func request_move(monster: SimMonster3D) -> void:
	var scanners: Array[NaiveScanner3D] = [
		NaiveScanners3D.AdjacentIslandScanner.new(),
		NaiveScanners3D.CheatScanner.new(),
		NaiveScanners3D.IslandExpansionScanner.new(),
		NaiveScanners3D.IslandMoatScanner.new(),
		NaiveScanners3D.PoolScanner.new(),
		NaiveScanners3D.WallExpansionScanner.new(),
		NaiveScanners3D.UnreachableScanner.new(),
	]
	
	for scanner: NaiveScanner3D in scanners:
...
func cancel_request(monster: SimMonster3D) -> void:
...
static func find_instance(node: Node) -> NaiveSolver3D:
